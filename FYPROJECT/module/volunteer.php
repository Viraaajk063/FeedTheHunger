...
<head>
  ...
  <title>Volunteeer-Feed The Hunger</title>
</head>
...
<body>
  ...
  <section class="secondsection" id="donation">
    <div class="request">
      <h3>Active Donation</h3>

      <?php
      require '../php/connectdb.php';
      $pincode = $_SESSION['user_pincode'];

      $query = "select * from donation_table where dpincode = '$pincode' AND status = 'accepted'";
      ...
      if ($num >= 1) {
        ...
        $query = "select request_id,vol_id from donation_table where dpincode = '$pincode' AND status = 'accepted'";
        $r_id = array();
        $result0 = mysqli_query($conn, $query);
        while ($row0 = mysqli_fetch_assoc($result0)) {
          $vol_id = unserialize($row0["vol_id"]);
          array_push($r_id,$row0["request_id"]);
          print_r($vol_id);
          if (in_array($user_id, $vol_id)) {
            
            //ddate,dtime,dpincode,dsecond_mob,dquantity,ddescript,requested_at
            $query = "select * FROM donation_table,usertable WHERE donation_table.user_id=usertable.id AND donation_table.dpincode='$pincode' AND donation_table.status='accepted'";
            $result = mysqli_query($conn, $query);
            $srno = 0;
